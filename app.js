...
window.onload = function () {
		...
		var eventCallback = {
			...
			element: function(data) {
				...
				if (data.task == 'disable') { document.querySelector('#'+data.value).style.display = 'none'; }
			 },
				...
				if (data.status == true) {
					if (vehicleInfo.classList.contains('inactive')) {
						...
						if (vehiclesCars.indexOf(data.type) > -1) {
							...
							document.querySelector('#fundo2').style.display = 'none';
							...
							document.querySelector('#fundo2').style.display = 'block';

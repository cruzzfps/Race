	...
	while true do
		...
		if Config.ui.showHealth == true then
			...
			playerStatus['status'][showPlayerStatus] = {
				...
				value = (GetEntityHealth(PlayerPedId())-100)/300*100
			}
		...
		TriggerServerEvent('papakko_hud:getServerInfo')
...
-- Overall Info
RegisterNetEvent('papakko_hud:setInfo')
AddEventHandler('papakko_hud:setInfo', function(info)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsControlJustPressed(1, Keys[Config.vehicle.keys.signalLeft]) and (has_value(vehiclesCars, vehicleClass) == true) then
			...
			TriggerEvent('papakko_hud:setCarSignalLights', vehicleSignalIndicator)
		end
...
AddEventHandler('papakko_hud:setCarSignalLights', function(status)
	...
	TriggerServerEvent('papakko_hud:syncCarLights', status)
...
RegisterNetEvent('papakko_hud:syncCarLights')
AddEventHandler('papakko_hud:syncCarLights', function(driver, status)
